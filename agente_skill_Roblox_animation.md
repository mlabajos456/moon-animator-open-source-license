Sí. Te dejo el **README oficial reescrito**, ya con enfoque **universal**, directo y listo para pegar como base maestra del skill. Está construido sobre el protocolo biomecánico ya confirmado en tu documento actual y actualizado con la nueva visión de motor universal de animación corporal.
...
**Actualización oficial V6 — criterio para golpes rectos estilo Roblox:**

En golpes rectos, `LowerArm` no debe usarse como driver principal de extensión. La extensión visual del golpe debe nacer en `UpperArm`, mientras `LowerArm` solo ajusta la línea final del antebrazo. En el rig R15 validado, usar `LowerArm.x` positivo como extensión principal puede provocar una flexión descendente del antebrazo y romper la lectura frontal del puño. Por tanto, en golpes rectos:

- `LowerArm.x` debe mantenerse cercano a `0` durante la salida e impacto.
- `LowerArm.z` puede usarse como microajuste de alineación del antebrazo en el impacto.
- `LowerArm.y` debe mantenerse mínimo salvo que se valide visualmente.
- si el golpe se ve rígido, primero se ajusta `UpperArm`, torso y apoyo; no se fuerza `LowerArm.x`.
...
# Actualización oficial V6 — Regla reusable para golpes rectos R15 estilo Roblox

Esta actualización se agrega como regla general del skill. No debe tratarse como una animación fija ni como un JSON que se copia literalmente. Su objetivo es enseñar al motor cómo construir golpes rectos de forma reutilizable, limpia y compatible con el rig R15.

## Principio central del golpe recto

Un golpe recto R15 estilo Roblox debe construirse a partir de una cadena clara:

**torso acompaña -> `UpperArm` conduce -> `LowerArm` alinea -> `Hand` refina**

La lectura visual del golpe debe sentirse como un bloque rígido y directo, propio del estilo Roblox, sin que el antebrazo se doble hacia abajo ni que la muñeca intente dirigir el movimiento.

## Regla de driver principal

El driver principal del golpe es siempre `UpperArm`.

Para golpe derecho:

- `RightUpperArm` conduce la dirección, profundidad, altura y lectura principal del golpe.
- `RightLowerArm` no lidera la extensión.
- `RightHand` solo refina el puño.

Para golpe izquierdo:

- `LeftUpperArm` conduce la dirección, profundidad, altura y lectura principal del golpe.
- `LeftLowerArm` no lidera la extensión.
- `LeftHand` solo refina el puño.

## Regla crítica sobre `LowerArm.x`

En el rig R15 validado, `LowerArm.x` positivo puede doblar el antebrazo hacia abajo cuando el brazo está proyectado al frente.

Por tanto, para golpes rectos:

- evitar usar `RightLowerArm.x` positivo como extensión del golpe.
- evitar usar `LeftLowerArm.x` positivo como extensión del golpe.
- mantener `LowerArm.x` cercano a `0` durante el impacto.
- usar `LowerArm.x` solo para recogida o microflexión si la lectura visual lo permite.
- si el antebrazo cae o se dobla, corregir reduciendo `LowerArm.x` antes de tocar otros ejes.

## Regla de alineación con `LowerArm.z`

En golpes rectos, `LowerArm.z` puede funcionar como eje de alineación del antebrazo y del puño durante el impacto.

Uso recomendado:

- mantener `LowerArm.z` bajo o neutro en guardia.
- aumentar o ajustar `LowerArm.z` en pre-impacto e impacto para limpiar la línea del antebrazo.
- retirar el ajuste de `LowerArm.z` en recoil y recuperación.
- validar visualmente el signo por cada lado; no asumir espejo perfecto.

Para golpe derecho:

- `RightLowerArm.z` puede participar como ajuste de impacto.
- `RightLowerArm.x` debe permanecer cercano a `0`.

Para golpe izquierdo:

- `LeftLowerArm.z` debe aplicarse como espejo funcional validado del golpe derecho.
- `LeftLowerArm.x` debe permanecer cercano a `0`.

## Regla de `Hand`

`Hand` nunca debe conducir el golpe.

En golpes rectos:

- `Hand` solo ajusta orientación de puño.
- los valores de `Hand` deben ser pequeños.
- si el puño se ve torcido, primero revisar `UpperArm` y `LowerArm.z`.
- evitar compensar un brazo mal alineado con rotaciones grandes de `Hand`.

## Regla de torso y soporte

El golpe recto necesita compensación corporal para no parecer un brazo flotando.

La cadena axial debe actuar así:

- `LowerTorso` estabiliza y empuja ligeramente la masa corporal.
- `UpperTorso` acompaña el giro hacia el lado del golpe.
- `Head` acompaña de forma mínima, sin exagerar.
- la pierna contraria puede cargar o sostener el impulso.
- el brazo contrario queda como guardia o contrapeso.

## Timing recomendado para golpe recto

Un golpe recto funcional puede construirse con esta secuencia:

- **Frame 0:** guardia/base estable.
- **Frame 5-6:** anticipación; torso carga ligeramente hacia atrás o hacia el lado contrario.
- **Frame 10-12:** salida; `UpperArm` empieza la proyección.
- **Frame 15-18:** impacto; `UpperArm` alcanza la lectura principal y `LowerArm.z` limpia la línea.
- **Frame 20-22:** hold corto; conservar impacto sin sobreanimar.
- **Frame 24-28:** recoil; retirar el golpe sin romper el antebrazo.
- **Frame 34-40:** recuperación a guardia/base.

## Regla de exclusión para golpes rectos

Evitar estas construcciones:

- usar `LowerArm.x` como si fuera el motor de la extensión.
- usar `Hand` como driver del golpe.
- compensar una mala línea del brazo con muñeca exagerada.
- girar `UpperArm.y` de forma extrema sin validación visual.
- usar posiciones grandes en `UpperArm` si el brazo se desengancha del cuerpo.
- copiar un JSON de golpe completo como regla universal.

## Regla de aprendizaje del skill

El golpe derecho validado sirve como referencia técnica, no como plantilla rígida.

El skill debe aprender el patrón:

- `UpperArm` define la trayectoria.
- `LowerArm.x` se controla para no doblar hacia abajo.
- `LowerArm.z` limpia impacto.
- `Hand` refina.
- torso y apoyo hacen que el golpe tenga peso.
- el espejo izquierdo se genera por regla funcional y se valida visualmente.

## Estado oficial

**Estado actual:** regla V6 incorporada como criterio oficial para construir golpes rectos R15 estilo Roblox.

**Uso recomendado:** golpes rectos, jabs, puñetes frontales, empujes rápidos y acciones de brazo rígido.

**Advertencia:** esta regla no reemplaza la validación visual. Si el rig cambia o el eje responde distinto, prevalece la lectura visual real.
...
- `CIERRE_LEVE_V1`
- `RECUPERACION_A_NEUTRO_V1`
- regla V6 para golpes rectos R15 estilo Roblox
